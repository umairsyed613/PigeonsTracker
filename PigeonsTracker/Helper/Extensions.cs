...
public static class Extensions
{
    public static TimeSpan Sum(this IEnumerable<TimeSpan> timeSpans)
    {
        ...
        {
            sumTillNowTimeSpan = sumTillNowTimeSpan.Add(timeSpan);
        }
    ...
    {
        return
            $"{((int)t.TotalHours).ToString("D2", CultureInfo.InvariantCulture)}:{t.Minutes.ToString("D2", CultureInfo.InvariantCulture)}:{t.Seconds.ToString("D2", CultureInfo.InvariantCulture)}";
    }
    ...
    public static (int hour, int minute, int second) GetSeparatedTime(this DateTime date)
    {
        /*
          ...
          our is less than 0 or greater than 23.

          inute is less than 0 or greater than 59.

          econd is less than 0 or greater than 59.
*/
        var hour = DateTime.Now.Hour;

        if (date.Hour is >= 0 and <= 23)
        {
            hour = date.Hour;
        }
